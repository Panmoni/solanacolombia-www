...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const project = await db
      .prepare('SELECT id, owner_wallet FROM projects WHERE id = ?')
      .bind(projectId)
      .first();
    ...
    const existingLink = await db
      .prepare(
        'SELECT id, status FROM builder_project_links WHERE builder_wallet = ? AND project_id = ?',
      )
      .bind(wallet, projectId)
      .first();
    ...
    `)
      .bind(linkId, wallet, projectId, now, now, now)
      .run();
